...
++  manual-tests
  ...
  :~  :+  %boot-bud
    ...
    :: |=  pin=ph-input
    :: ?:  =(%init -.q.uf.pin)
    ::   [& (dojo ~bud "|start %btc-node-store") %wait ~]
    :: ?:  (is-dojo-output ~bud who.pin uf.pin "done")
    ::   [& (dojo ~bud "|start %btc-node-store") %wait ~
    ::   [& ~ %done ~]
    :: [& ~ %wait ~]
    ...
    ::
    :: ;<  ~  bind:m  (just-events (dojo ~bud ":btc-node-hook [%create-wallet 'test2' ~ ~]"))
    :: ;<  ~  bind:m  (wait-for-dojo ~bud "create-wallet")
    :: ::
    ;<  ~  bind:m  (just-events (dojo ~bud ":btc-node-hook [%get-balance 'test2' ~ ~ ~]"))
    ;<  ~  bind:m  (wait-for-dojo ~bud "get-balance")
    ::
    ;<  ~  bind:m  (just-events (dojo ~bud ":btc-node-hook [%list-wallets ~]"))
    ;<  ~  bind:m  (wait-for-dojo ~bud "list-wallets")
    ::
    ;<  ~  bind:m  (just-events (dojo ~bud ":btc-node-hook [%list-transactions ~ ~ ~ ~]"))
